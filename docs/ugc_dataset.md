...
| 手机色彩 `ugc/look.py` | 白平衡、曝光、高光软削顶、暗部提亮、S 曲线、饱和度（YCrCb） | 分布匹配：联合抽取一个真实镜头的五项统计作为目标，在预览帧上迭代 6 次逼近。匹配后中位数：饱和度 67/74、色温 18/21、亮度 119/124、暗部 29/28、高光 204/211（我们 / 真实） |
...
- **校准样本小**：只有 30 条广告；服务器上应该用几百条广告复核一次（`adup.analysis.compare`）。
